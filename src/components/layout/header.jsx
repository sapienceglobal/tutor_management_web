...
import { C, T, R, S } from '@/constants/studentTokens'; // Sourced from single truth
...
export function Header({ onMenuClick, onSidebarCollapse, isSidebarCollapsed, institute }) {
    ...
    useEffect(() => {
        ...
        const delayDebounceFn = setTimeout(async () => {
            ...
            try {
                ...
                console.error("Error in Unified Header Search:", err);
            } finally {
    ...
    return (
        <header className="sticky top-0 z-40 w-full backdrop-blur-sm shadow-[0_1px_3px_0_rgba(0,0,0,0.04)]"
            style={{ backgroundColor: 'rgba(255, 255, 255, 0.95)', borderBottom: `1px solid ${C.cardBorder}`, height: '60px' }}>
            <div className="h-full flex items-center justify-between px-4 lg:px-5 gap-3">
                ...
                <div ref={searchContainerRef} className="hidden md:block flex-1 max-w-[420px] ml-4 relative z-50">
                    <div className="relative group">
                        ...
                        <input
                            ...
                            placeholder={
                                userRole === 'superadmin' 
                                    ? "Search institutes, users..." 
                                    : userRole === 'admin' 
                                        ? "Search courses, tutors, students, batches..." 
                                        : "Search assigned courses, batches..."
                            }
                            value={searchTerm}
                            onChange={e => {
                        ...
                        >
                            {isSearching ? (
                                ...
                                <div className="flex flex-col gap-4">
                                    {/* 1. Institutes Category (Super Admin) */}
                                    {searchResults.institutes?.length > 0 && (
                                        <div>
                                            <div style={{ fontFamily: T.fontFamily, fontSize: '10px', fontWeight: T.weight.bold, color: C.btnPrimary, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                <span>🏢 Platform Institutes</span>
                                                <span style={{ backgroundColor: `${C.btnPrimary}15`, color: C.btnPrimary, padding: '1px 6px', borderRadius: '6px', fontSize: '9px' }}>{searchResults.institutes.length}</span>
                                            </div>
                                            <div className="flex flex-col gap-1">
                                                {searchResults.institutes.map(inst => (
                                                    <Link key={inst._id} href="/superadmin/institutes" onClick={() => setShowResults(false)} className="flex items-center gap-3 p-2 transition-all hover:bg-gray-50 rounded-lg text-decoration-none">
                                                        <div style={{ width: '36px', height: '36px', borderRadius: '6px', overflow: 'hidden', backgroundColor: `${C.btnPrimary}10`, display: 'flex', alignItems: 'center', justifyCenter: 'center', flexShrink: 0 }}>
                                                            {inst.logo ? (
                                                                <img src={resolveImageUrl(inst.logo)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                                            ) : (
                                                                <span style={{ fontSize: '16px', color: C.btnPrimary }}>🏢</span>
                                                            )}
                                                        </div>
                                                        <div style={{ minWidth: 0, flex: 1 }}>
                                                            <p style={{ margin: 0, fontFamily: T.fontFamily, fontSize: '13px', fontWeight: T.weight.bold, color: C.heading }}>{inst.name}</p>
                                                            <p style={{ margin: 0, fontFamily: T.fontFamily, fontSize: '10px', color: C.textMuted }}>{inst.subdomain}.sapience.com</p>
                                                        </div>
                                                    </Link>
                                                ))}
                                            </div>
                                    ...
                                    {/* 2. Users Category (Super Admin / Admin) */}
                                    {searchResults.users?.length > 0 && (
                                        <div>
                                            <div style={{ fontFamily: T.fontFamily, fontSize: '10px', fontWeight: T.weight.bold, color: C.btnPrimary, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                <span>👥 Global Users</span>
                                                <span style={{ backgroundColor: `${C.btnPrimary}15`, color: C.btnPrimary, padding: '1px 6px', borderRadius: '6px', fontSize: '9px' }}>{searchResults.users.length}</span>
                                            </div>
                                            <div className="flex flex-col gap-1">
                                                {searchResults.users.map(u => (
                                                    <Link key={u._id} href="/superadmin/users" onClick={() => setShowResults(false)} className="flex items-center gap-3 p-2 transition-all hover:bg-gray-50 rounded-lg text-decoration-none">
                                                        <div style={{ width: '36px', height: '36px', borderRadius: '6px', overflow: 'hidden', backgroundColor: '#f3f4f6', flexShrink: 0 }}>
                                                            <img src={resolveImageUrl(u.profileImage)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                                        </div>
                                                        <div style={{ minWidth: 0, flex: 1 }}>
                                                            <p style={{ margin: 0, fontFamily: T.fontFamily, fontSize: '13px', fontWeight: T.weight.bold, color: C.heading }}>{u.name}</p>
                                                            <p style={{ margin: 0, fontFamily: T.fontFamily, fontSize: '10px', color: C.textMuted }}>{u.email} • <span className="capitalize">{u.role}</span></p>
                                                        </div>
                                                    </Link>
                                                ))}
                                            </div>
                                    ...
                                    {/* 3. Courses Category (Admin / Tutor) */}
                                    ...
                                    {/* 4. Tutors Category (Admin) */}
                                    {searchResults.tutors?.length > 0 && (
                                        <div>
                                            <div style={{ fontFamily: T.fontFamily, fontSize: '10px', fontWeight: T.weight.bold, color: C.btnPrimary, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                <span>👨‍🏫 Tutors & Instructors</span>
                                                <span style={{ backgroundColor: `${C.btnPrimary}15`, color: C.btnPrimary, padding: '1px 6px', borderRadius: '6px', fontSize: '9px' }}>{searchResults.tutors.length}</span>
                                            </div>
                                            <div className="flex flex-col gap-1">
                                                {searchResults.tutors.map(t => (
                                                    <Link key={t._id} href="/admin/tutors" onClick={() => setShowResults(false)} className="flex items-center gap-3 p-2 transition-all hover:bg-gray-50 rounded-lg text-decoration-none">
                                                        <div style={{ width: '36px', height: '36px', borderRadius: '6px', overflow: 'hidden', backgroundColor: '#f3f4f6', flexShrink: 0 }}>
                                                            <img src={resolveImageUrl(t.profileImage)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                                        </div>
                                                        <div style={{ minWidth: 0, flex: 1 }}>
                                                            <p style={{ margin: 0, fontFamily: T.fontFamily, fontSize: '13px', fontWeight: T.weight.bold, color: C.heading }}>{t.name}</p>
                                                            <p style={{ margin: 0, fontFamily: T.fontFamily, fontSize: '10px', color: C.textMuted }}>{t.email}</p>
                                                        </div>
                                                    </Link>
                                                ))}
                                            </div>
                                    ...
                                    {/* 5. Students Category (Admin / Tutor) */}
                                    ...
                                    {/* 6. Batches Category (Admin / Tutor) */}
                                            ...
                                            <div className="flex flex-col gap-1">
                                                {searchResults.batches.map(b => (
                                                    <Link key={b._id} href={`/${userRole}/batches`} onClick={() => setShowResults(false)} className="flex items-center gap-3 p-2 transition-all hover:bg-gray-50 rounded-lg text-decoration-none">
                                                        <div style={{ width: '36px', height: '36px', borderRadius: '6px', overflow: 'hidden', backgroundColor: `${C.warningBg}80`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0 ml-auto">
                    ...
                    <button onClick={handleFullscreen} title="Toggle fullscreen"
                        className="hidden lg:flex items-center justify-center p-2 transition-colors border-none cursor-pointer"
                        style={{ backgroundColor: 'transparent', color: C.textMuted, borderRadius: '10px' }}
                        onMouseEnter={e => { e.currentTarget.style.backgroundColor = C.innerBg; e.currentTarget.style.color = C.heading; }}
                        onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = C.textMuted; }}>
                    ...
                    <div className="relative" ref={dropdownRef}>
                        ...
                        >
                            ...
                            <div className="hidden sm:flex flex-col items-start min-w-0" suppressHydrationWarning>
                                ...
                                <span className="leading-tight capitalize" style={{ fontFamily: T.fontFamily, fontSize: '10px', fontWeight: T.weight.semibold, color: C.textMuted }}>
                                    {roleLabel}
                                </span>
                            </div>
                            <MdKeyboardArrowDown className={`hidden sm:block transition-transform duration-200 ${isProfileOpen ? 'rotate-180' : ''}`} style={{ width: 16, height: 16, color: C.textMuted }} />
                        </button>

                        {isProfileOpen && (
                            <div className="absolute right-0 mt-2 w-56 py-1.5 z-50 animate-in fade-in slide-in-from-top-2 duration-150"
                                ...
                                <div className="px-4 py-3" style={{ borderBottom: `1px solid ${C.cardBorder}` }}>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="min-w-0">
                                            <p className="truncate" style={{ fontFamily: T.fontFamily, fontSize: T.size.base, fontWeight: T.weight.bold, color: C.heading, margin: '0 0 2px 0' }}>{user?.name || 'Admin'}</p>
                                            <p className="truncate" style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, color: C.textMuted, margin: 0 }}>{user?.email || 'admin@example.com'}</p>
                                            {institute ? (
                                                ...
                                                <p style={{ fontFamily: T.fontFamily, fontSize: '10px', color: C.textMuted, margin: '2px 0 0 0' }}>No institute assigned</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="py-1">
                                    <button
                                        ...
                                        className="w-full flex items-center gap-2 px-4 py-2 cursor-pointer border-none text-left"
                                        style={{ backgroundColor: 'transparent', fontFamily: T.fontFamily, fontSize: T.size.base, fontWeight: T.weight.semibold, color: C.text }}
                                        onMouseEnter={e => e.currentTarget.style.backgroundColor = C.innerBg}
                                        onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}>
                                    ...
                                    <button
                                        ...
                                        className="w-full flex items-center gap-2 px-4 py-2 cursor-pointer border-none text-left"
                                        style={{ backgroundColor: 'transparent', fontFamily: T.fontFamily, fontSize: T.size.base, fontWeight: T.weight.bold, color: C.danger }}
