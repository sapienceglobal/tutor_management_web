...
import {
    ...
    ChevronDown, RotateCcw, ImagePlus, X, Clock,
...
function PastDoubtCard({ doubt, onClick, isActive }) {
    return (
        <div
            ...
            style={{
                backgroundColor: isActive ? 'rgba(59,130,246,0.08)' : '#fff',
                border: isActive ? '1px solid rgba(59,130,246,0.35)' : '1px solid rgba(59,130,246,0.12)',
                boxShadow: isActive ? '0 0 0 2px rgba(59,130,246,0.12)' : '0 2px 8px rgba(59,130,246,0.06)',
            }}
            onMouseEnter={e => { if (!isActive) e.currentTarget.style.boxShadow = '0 4px 16px rgba(59,130,246,0.14)'; }}
            onMouseLeave={e => { if (!isActive) e.currentTarget.style.boxShadow = '0 2px 8px rgba(59,130,246,0.06)'; }}
        >
            <p className="line-clamp-2" style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, fontWeight: T.weight.semibold, color: '#334155', lineHeight: 1.4 }}>
                {doubt.question}
            </p>
            <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                {doubt.subject && (
                    <span className="px-1.5 py-0.5 rounded-full"
                        style={{ backgroundColor: 'rgba(59,130,246,0.10)', fontFamily: T.fontFamily, fontSize: '10px', fontWeight: T.weight.bold, color: '#3B82F6' }}>
                        {doubt.subject}
                    </span>
                )}
                {doubt.difficulty && (
                ...
                <span style={{ fontFamily: T.fontFamily, fontSize: '10px', color: '#94A3B8', marginLeft: 'auto' }}>{doubt.timeAgo}</span>
            </div>
        </div>
    );
}
...
export default function DoubtSolverPage() {
    ...
    const [topics, setTopics]               = useState([]);        // taxonomy topics
    const [usedSubjects, setUsedSubjects]   = useState([]);        // from doubt history
    ...
    const [currentDoubt, setCurrentDoubt]   = useState(null); // { doubtLogId, question, answer, subject, difficulty }
    ...
    // ── Merged subject list: taxonomy + used subjects ───────────────
    const allSubjects = [
        ...topics.map(t => t.name),
        ...usedSubjects.filter(s => !topics.find(t => t.name === s)),
    ].sort();
    ...
    useEffect(() => {
        const init = async () => {
            setLoadingInit(true);
            try {
                const [courseRes, topicRes, historyRes, statsRes] = await Promise.all([
                    api.get('/courses/my-courses'),
                    api.get('/ai/doubt-topics'),
                    api.get('/ai/doubts?limit=6'),
                ...
                if (topicRes.data?.success) {
                    ...
                    setUsedSubjects(topicRes.data.usedSubjects || []);
                }
                ...
                toast.error('Failed to load Doubt Solver');
            } finally {
    ...
    // ── Solve doubt ─────────────────────────────────────────────────
    const solveDoubt = async (forcedQ = null, forcedSubject = null) => {
        ...
        try {
            const res = await api.post('/ai/solve-doubt', {
                question:   q,
                subject:    forcedSubject || subject || undefined,
            ...
            if (res.data?.success) {
                ...
                // Refresh past doubts list
                const histRes = await api.get('/ai/doubts?limit=6');
                if (histRes.data?.success) setPastDoubts(histRes.data.doubts || []);
    ...
    const loadPastDoubt = async (doubtId) => {
        if (activeDoubtId === doubtId?.toString()) return; // already active
        try {
    ...
    return (
        <div className="flex gap-4 h-full p-4" style={{ fontFamily: T.fontFamily, backgroundColor: C.pageBg }}>
            ...
            {/* ── LEFT PANEL ──────────────────────────────────────────────── */}
            <div className="flex flex-col gap-3 w-[280px] flex-shrink-0 overflow-y-auto custom-scrollbar">

                {/* Header */}
                <div className="rounded-2xl p-4 flex-shrink-0"
                    style={{ background: 'linear-gradient(135deg,#1D4ED8 0%,#3B82F6 60%,#60A5FA 100%)', boxShadow: '0 6px 24px rgba(59,130,246,0.30)', position: 'relative', overflow: 'hidden' }}>
                    {[...Array(6)].map((_, i) => (
                ...
                {/* Question input */}
                <div className="rounded-2xl p-4 flex-shrink-0"
                    style={{ backgroundColor: '#fff', border: '1px solid rgba(59,130,246,0.12)', boxShadow: S.card }}>
                    <p style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, fontWeight: T.weight.bold, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>
                        Ask Your Doubt
                    </p>
                    <textarea
                        value={question}
                        ...
                        style={{ fontFamily: T.fontFamily, fontSize: T.size.sm, color: '#1E293B', backgroundColor: 'transparent', lineHeight: 1.6 }}
                    />

                    {imagePreview && (
                        <div className="mt-2 relative rounded-xl overflow-hidden" style={{ border: '1px solid rgba(59,130,246,0.15)' }}>
                            <img src={imagePreview} alt="doubt" className="w-full h-24 object-cover" />
                    ...
                    <div className="flex items-center justify-between mt-3">
                        <button onClick={() => fileRef.current?.click()}
                            ...
                            style={{ backgroundColor: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.12)' }}>
                            <ImagePlus className="w-3.5 h-3.5" style={{ color: '#3B82F6' }} />
                            <span style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, fontWeight: T.weight.semibold, color: '#3B82F6' }}>Image</span>
                        </button>
                ...
                <div className="rounded-2xl p-4 flex-shrink-0"
                    ...
                    {/* Subject — from taxonomy + doubt history */}
                    <div className="mb-3">
                        <label style={{ fontFamily: T.fontFamily, fontSize: '10px', color: '#94A3B8', fontWeight: T.weight.semibold }}>Subject / Topic</label>
                        <div className="relative mt-1">
                            {loadingInit ? <Skeleton h={8} /> : (
                                <>
                                    <select value={subject} onChange={e => setSubject(e.target.value)}
                                        className="w-full appearance-none px-3 py-2 rounded-xl pr-7 outline-none"
                                        style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, color: '#334155', border: '1px solid rgba(59,130,246,0.15)', backgroundColor: 'rgba(59,130,246,0.04)' }}>
                    ...
                    {/* Course */}
                    <div className="mb-3">
                        <label style={{ fontFamily: T.fontFamily, fontSize: '10px', color: '#94A3B8', fontWeight: T.weight.semibold }}>Course Context</label>
                        <div className="relative mt-1">
                            {loadingInit ? <Skeleton h={8} /> : (
                                <>
                    ...
                    {/* Difficulty */}
                    <div>
                        <label style={{ fontFamily: T.fontFamily, fontSize: '10px', color: '#94A3B8', fontWeight: T.weight.semibold }}>Difficulty</label>
                ...
                <div className="rounded-2xl p-4 flex-shrink-0"
                    ...
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <p style={{ fontFamily: T.fontFamily, fontSize: T.size.sm, fontWeight: T.weight.bold, color: '#1E293B' }}>Solver Stats</p>
                    </div>
                    {loadingInit ? (
                        <div className="space-y-2.5"><Skeleton h={5} /><Skeleton h={5} /><Skeleton h={5} /></div>
                    ) : (
                        [
                            { label: 'Doubts Solved',  value: stats?.doubtsSolved    ?? '0',   color: '#3B82F6' },
                            { label: 'Avg Response',   value: stats?.avgResponseTime ?? 'N/A', color: '#10B981' },
                            { label: 'AI Sessions',    value: stats?.totalSessions   ?? '0',   color: '#8B5CF6' },
                        ].map(st => (
                            <div key={st.label} className="flex items-center justify-between mb-2.5 last:mb-0">
                                <span style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, color: '#64748B' }}>{st.label}</span>
            ...
            <div className="flex-1 flex flex-col rounded-2xl overflow-hidden"
                style={{ backgroundColor: '#F0F7FF', border: '1px solid rgba(59,130,246,0.12)', boxShadow: '0 4px 24px rgba(59,130,246,0.08)' }}>

                {/* Topbar */}
                <div className="flex items-center justify-between px-5 py-3.5 flex-shrink-0"
                    style={{ backgroundColor: '#fff', borderBottom: '1px solid rgba(59,130,246,0.10)' }}>
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                            style={{ background: 'linear-gradient(135deg,#1D4ED8,#3B82F6)', boxShadow: '0 2px 8px rgba(59,130,246,0.35)' }}>
                            <Sparkles className="w-5 h-5 text-white" />
                        </div>
                        <div>
                            <p style={{ fontFamily: T.fontFamily, fontSize: T.size.base, fontWeight: T.weight.bold, color: '#1E293B' }}>AI Explanation</p>
                    ...
                    {currentDoubt && (
                        <button onClick={reset}
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl transition-all hover:opacity-80"
                            style={{ backgroundColor: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.12)' }}>
                            <RotateCcw className="w-3.5 h-3.5" style={{ color: '#3B82F6' }} />
                            <span style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, fontWeight: T.weight.semibold, color: '#3B82F6' }}>New Doubt</span>
                        </button>
                    )}
                </div>
                    ...
                    {!currentDoubt && !solving && (
                        <div className="h-full flex flex-col justify-center">
                            ...
                            <div className="rounded-2xl p-5 mb-4 flex items-center gap-4"
                                style={{ background: 'linear-gradient(135deg,#1D4ED8 0%,#3B82F6 60%,#60A5FA 100%)', boxShadow: '0 8px 32px rgba(59,130,246,0.25)', position: 'relative', overflow: 'hidden' }}>
                                {[...Array(8)].map((_, i) => (
                                    <div key={i} className="absolute rounded-full"
                            ...
                            {/* Topic-based quick starts — from taxonomy */}
                            {loadingInit ? (
                                ...
                                <>
                                    <p style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: T.weight.bold, marginBottom: 10 }}>
                                        Your Topics — Click to Solve
                                    </p>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
                                        {topics.slice(0, 4).map(topic => (
                                            <button key={topic.name}
                                                onClick={() => solveDoubt(`Explain ${topic.name}`, topic.name)}
                                                className="text-left px-4 py-3 rounded-xl transition-all hover:opacity-80 hover:-translate-y-0.5"
                                                style={{ backgroundColor: '#fff', border: '1px solid rgba(59,130,246,0.15)', boxShadow: '0 2px 8px rgba(59,130,246,0.06)', fontFamily: T.fontFamily, fontSize: T.size.xs, color: '#475569' }}>
                                                <div className="flex items-start gap-2">
                                                    <Lightbulb className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" style={{ color: '#3B82F6' }} />
                                                    <div>
                                                        <p style={{ fontWeight: T.weight.semibold, color: '#334155' }}>{topic.name}</p>
                                                        {topic.course && <p style={{ fontSize: '10px', color: '#94A3B8', marginTop: 2 }}>{topic.course}</p>}
                                                    </div>
                                                </div>
                                            </button>
                                        ))}
                                ...
                                <div className="mb-4 p-4 rounded-2xl text-center"
                                    style={{ backgroundColor: 'rgba(59,130,246,0.05)', border: '1px dashed rgba(59,130,246,0.20)' }}>
                                    <Layers className="w-8 h-8 mx-auto mb-2" style={{ color: 'rgba(59,130,246,0.35)' }} />
                                    <p style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, color: '#94A3B8' }}>
                                        Add topics in <strong>Taxonomy → Manage Topics</strong> to see quick starts here.
                                    </p>
                                </div>
                            )}
                            ...
                            <div className="rounded-2xl p-4"
                                style={{ backgroundColor: 'rgba(59,130,246,0.05)', border: '1px solid rgba(59,130,246,0.12)' }}>
                                <p style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, fontWeight: T.weight.bold, color: '#3B82F6', marginBottom: 8 }}>💡 Tips for better answers</p>
                                {[
                                    'Be specific — mention topic, chapter, or concept',
                                    'Set difficulty level for a targeted explanation',
                                    'Select a course for curriculum-aware answers',
                                    ...
                                    <div key={tip} className="flex items-center gap-2 mb-1.5 last:mb-0">
                                        ...
                                        <p style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, color: '#64748B' }}>{tip}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    {solving && (
                        <div className="flex flex-col items-center justify-center h-full gap-4">
                            ...
                            <div className="text-center">
                                ...
                                <p style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, color: '#94A3B8' }}>Powered by Groq · usually 2–5s</p>
                            </div>
                            <div className="flex items-center gap-1.5 px-4 py-3 rounded-2xl"
                                style={{ backgroundColor: '#fff', border: '1px solid rgba(59,130,246,0.12)' }}>
                                {[0, 1, 2].map(i => (
                                    <span key={i} className="w-2 h-2 rounded-full"
                                        style={{ backgroundColor: '#3B82F6', animation: `ds-bounce 1.2s ease-in-out ${i * 0.2}s infinite` }} />
                                ))}
                    ...
                    {currentDoubt && !solving && (
                        <div ref={answerRef} className="space-y-4">
                            ...
                            <div className="flex items-start gap-3 p-4 rounded-2xl"
                                style={{ backgroundColor: '#fff', border: '1px solid rgba(59,130,246,0.12)', boxShadow: '0 2px 8px rgba(59,130,246,0.06)' }}>
                                <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0"
                                    style={{ backgroundColor: 'rgba(59,130,246,0.10)' }}>
                                    <MessageSquare className="w-4 h-4" style={{ color: '#3B82F6' }} />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p style={{ fontFamily: T.fontFamily, fontSize: '10px', color: '#94A3B8', fontWeight: T.weight.bold, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Your Doubt</p>
                                    <p style={{ fontFamily: T.fontFamily, fontSize: T.size.sm, color: '#334155', marginTop: 2, lineHeight: 1.5 }}>
                            ...
                            <div className="p-5 rounded-2xl"
                                style={{ backgroundColor: '#fff', border: '1px solid rgba(59,130,246,0.14)', boxShadow: '0 4px 20px rgba(59,130,246,0.10)' }}>
                                <div className="flex items-center gap-2 mb-4">
                                    <div className="w-8 h-8 rounded-xl flex items-center justify-center"
                                        style={{ background: 'linear-gradient(135deg,#1D4ED8,#3B82F6)' }}>
                                        <Sparkles className="w-4 h-4 text-white" />
                                    </div>
                                    <p style={{ fontFamily: T.fontFamily, fontSize: T.size.sm, fontWeight: T.weight.black, color: '#1E293B' }}>AI Explanation</p>
                            ...
                            <div className="flex items-center gap-3 p-4 rounded-2xl"
                                style={{ backgroundColor: '#fff', border: '1px solid rgba(59,130,246,0.10)', boxShadow: '0 2px 8px rgba(59,130,246,0.06)' }}>
                                <AlertCircle className="w-4 h-4 flex-shrink-0" style={{ color: '#94A3B8' }} />
                                ...
                                {ratingLoading && <Loader2 className="w-3.5 h-3.5 animate-spin" style={{ color: '#94A3B8' }} />}
            ...
            <div className="flex flex-col gap-3 w-[240px] flex-shrink-0 overflow-y-auto custom-scrollbar">
                ...
                {/* Recent Doubts — real from DB */}
                <div className="rounded-2xl p-4 flex-shrink-0"
                    style={{ backgroundColor: '#fff', border: '1px solid rgba(59,130,246,0.12)', boxShadow: S.card }}>
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                            <Clock className="w-4 h-4" style={{ color: '#3B82F6' }} />
                            <p style={{ fontFamily: T.fontFamily, fontSize: T.size.sm, fontWeight: T.weight.bold, color: '#1E293B' }}>Recent Doubts</p>
                        </div>
                        {pastDoubts.length > 0 && (
                            <span style={{ fontFamily: T.fontFamily, fontSize: '10px', color: '#94A3B8' }}>
                                {pastDoubts.length} total
                            </span>
                        )}
                    </div>
                    ...
                    {loadingInit ? (
                        ...
                        <div className="text-center py-6">
                            <HelpCircle className="w-8 h-8 mx-auto mb-2" style={{ color: 'rgba(59,130,246,0.25)' }} />
                ...
                {/* Topics from Taxonomy — real from DB */}
                <div className="rounded-2xl p-4 flex-shrink-0"
                    style={{ backgroundColor: '#fff', border: '1px solid rgba(59,130,246,0.12)', boxShadow: S.card }}>
                    <div className="flex items-center gap-2 mb-3">
                        <Tag className="w-4 h-4" style={{ color: '#3B82F6' }} />
                        <p style={{ fontFamily: T.fontFamily, fontSize: T.size.sm, fontWeight: T.weight.bold, color: '#1E293B' }}>Your Topics</p>
                    </div>

                    {loadingInit ? (
                        <div className="flex flex-wrap gap-1.5">
                            {[...Array(5)].map((_, i) => <Skeleton key={i} h={6} w="20" rounded="full" />)}
                        </div>
                    ) : topics.length === 0 && usedSubjects.length === 0 ? (
                        <p style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, color: '#CBD5E1', textAlign: 'center', padding: '12px 0' }}>
                            No topics yet. Add in Taxonomy.
                        </p>
                    ) : (
                        <div className="flex flex-wrap gap-1.5">
                            {[...topics.map(t => t.name), ...usedSubjects].map(name => (
                                <button key={name} onClick={() => handleTopicClick(name)}
                                    className="px-2.5 py-1 rounded-full transition-all hover:opacity-80"
                                    style={{ backgroundColor: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.14)', fontFamily: T.fontFamily, fontSize: '10px', fontWeight: T.weight.semibold, color: '#3B82F6' }}>
                                    {name}
                                </button>
                            ))}
                        </div>
                ...
                {/* AI status */}
                <div className="rounded-2xl p-4"
                    style={{ background: 'linear-gradient(135deg,rgba(59,130,246,0.08),rgba(99,102,241,0.08))', border: '1px solid rgba(59,130,246,0.14)' }}>
                    <div className="flex items-center gap-2 mb-1">
                        <Sparkles className="w-4 h-4" style={{ color: '#6366F1' }} />
                        <p style={{ fontFamily: T.fontFamily, fontSize: T.size.xs, fontWeight: T.weight.black, color: '#4F46E5' }}>Powered by Groq AI</p>
                    </div>
