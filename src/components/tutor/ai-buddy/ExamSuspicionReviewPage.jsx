...
import {
    ...
    MessageSquare, AlertOctagon, HelpCircle, User, Loader2, Sparkles
} from 'lucide-react';
...
export default function ExamSuspicionReviewPage() {
    ...
    const handleGenerateSummary = async () => {
        ...
        try {
            const res = await api.post(`/ai/proctoring/review/${params.attemptId}/summary`);
            if (res.data?.success) {
                setData(prev => ({
                    ...prev,
                    aiProctoringSummary: res.data.summary
                }));
                toast.success('AI Proctoring summary generated successfully!');
            }
        } catch (err) {
    ...
    const handleBottomAction = (label) => {
        ...
        let loadingMsg = 'Processing academic action...';
        let successMsg = 'Action applied successfully!';

        if (label === 'Escalate Flags') {
            loadingMsg = 'Filing official academic integrity escalation to administration...';
            successMsg = 'Suspicion attempt escalated to Exam Integrity Committee successfully!';
        } else if (label === 'Dismiss Flags') {
            loadingMsg = 'Dismissing flagged integrity records for this student...';
            successMsg = 'Proctoring violation alerts dismissed successfully!';
        } else if (label === 'Request Assistance') {
            loadingMsg = 'Reaching out to IT Proctors for immediate support...';
            successMsg = 'Support ticket created! An IT proctor will review this session shortly.';
        } else if (label === 'Generate Report') {
            loadingMsg = 'Compiling proctor logs, violations, and student metrics...';
            successMsg = 'PDF integrity check report successfully generated and saved!';
        } else if (label === 'Finalize Review') {
            loadingMsg = 'Locking proctor review audit trail for this attempt...';
            successMsg = 'Session marked as reviewed! Finalized integrity state saved.';
        }
            ...
            <div className="flex flex-col h-full overflow-y-auto custom-scrollbar p-5 gap-5"
                ...
                <div className="rounded-2xl p-5 bg-white flex flex-col gap-4"
                    ...
                    <div className="p-4 rounded-xl flex items-center justify-between"
                        ...
                        <button
                            onClick={() => handleBottomAction('Escalate Flags')}
                            className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-white font-bold text-sm transition-opacity hover:opacity-90 shadow-lg"
                    ...
                    <div className="lg:col-span-5 flex flex-col gap-5 min-h-0">
                        ...
                        <div className="rounded-2xl bg-white p-5 border flex-1"
                            ...
                            <div className="mt-6 p-4 rounded-xl border border-slate-100 bg-slate-50">
                                ...
                                {data.aiProctoringSummary && (
                                    <div className="mt-4 p-3 rounded-xl border border-purple-100 bg-purple-50/50">
                                        <h5 className="flex items-center gap-1.5 text-xs font-black text-purple-700 uppercase tracking-wider mb-1.5">
                                            <Sparkles className="w-3.5 h-3.5" /> AI Proctoring Summary
                                        </h5>
                                ...
                                <button
                                    ...
                                    className="w-full mt-5 py-2.5 rounded-xl font-bold text-white text-sm transition-opacity hover:opacity-90 shadow-md flex items-center justify-center gap-2"
                                    style={{ background: P.gradient, opacity: generatingSummary ? 0.7 : 1 }}>
                                    {generatingSummary ? (
                                        <>
                                        ...
                                        <>
                                            ...
                                            <span>{data.aiProctoringSummary ? 'Regenerate Summary' : 'Generate Summary'}</span>
                                        </>
                                    )}
                                </button>
                            </div>
                ...
                <div className="flex items-center justify-between p-4 rounded-2xl bg-white sticky bottom-0 border mt-auto"
                    ...
                    <div className="flex gap-3">
                        <button
                            onClick={() => handleBottomAction('Escalate Flags')}
                            className="px-5 py-2 rounded-xl text-sm font-bold bg-purple-50 text-purple-700 hover:bg-purple-100 border border-purple-100 transition-colors flex items-center gap-2">
                            <AlertTriangle className="w-4 h-4" /> Escalate Flags
                        </button>
                        <button
                            onClick={() => handleBottomAction('Dismiss Flags')}
                            className="px-5 py-2 rounded-xl text-sm font-bold bg-slate-50 text-slate-600 hover:bg-slate-100 border border-slate-200 transition-colors flex items-center gap-2">
                            <CheckCircle2 className="w-4 h-4" /> Dismiss Flags
                        </button>
                        <button
                            onClick={() => handleBottomAction('Request Assistance')}
                            className="px-5 py-2 rounded-xl text-sm font-bold bg-orange-50 text-orange-600 hover:bg-orange-100 border border-orange-200 transition-colors flex items-center gap-2">
                            <HelpCircle className="w-4 h-4" /> Request Assistance
                        </button>
                    </div>
                    <div className="flex gap-3">
                        <button
                            onClick={() => handleBottomAction('Generate Report')}
                            className="px-6 py-2 rounded-xl text-sm font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 transition-colors flex items-center gap-2">
                            <FileText className="w-4 h-4" /> Generate Report
                        </button>
                        <button
                            onClick={() => handleBottomAction('Finalize Review')}
                            className="px-6 py-2 rounded-xl text-sm font-bold text-white transition-opacity hover:opacity-90 flex items-center gap-2 shadow-md"
                            style={{ background: P.gradient }}>
