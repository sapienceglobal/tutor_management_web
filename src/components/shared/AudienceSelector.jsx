...
export default function AudienceSelector({
  ...
  const updateAudience = (patch) => {
    ...
    if (
      ["institute", "batch", "private"].includes(next.scope) &&
      !next.instituteId
    ) {
      next.instituteId = instituteId || institute?._id || null;
    }
    onChange(next);
  };

  // Auto-correct invalid scope on mount/load for independent tutors

  useEffect(() => {
    if (audience.scope === "institute" && !institute && !instituteId) {
      updateAudience({ scope: allowGlobal ? "global" : "batch" });
    }
  ...
  const toggleArrayValue = (key, id) => {
    ...
    updateAudience({
      [key]: exists ? list.filter((e) => e !== id) : [...list, id],
    });
  };
              ...
              {availableBatches.map((batch) => (
                ...
                >
                  ...
                  <Switch
                    ...
                    onCheckedChange={() =>
                      toggleArrayValue("batchIds", batch._id)
                    }
                  />
                </label>
              ))}
              ...
              {availableStudents.map((student) => (
                ...
                >
                  ...
                  <Switch
                    ...
                    onCheckedChange={() =>
                      toggleArrayValue("studentIds", student._id)
                    }
                  />
                </label>
              ))}
