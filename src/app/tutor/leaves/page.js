...
import { Calendar, Plus, Clock, CheckCircle2, XCircle, FileText, Paperclip, UserCheck, Loader2 } from 'lucide-react';
...
export default function UserLeavesPage() {
    ...
    const fetchTutors = async () => {
        try {
            ...
            // We just need tutor names for dropdown — reuse the students endpoint or a separate one
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                setShowForm(false);
                setFormData({ startDate: '', endDate: '', reason: '', documentUrl: '', substituteId: '' });
                fetchMyLeaves();
            }
        } catch (error) {
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-800"
                                />
                                <p className="text-xs text-slate-400">Upload to Google Drive/Cloudinary and paste the link</p>
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-slate-700 flex items-center gap-1.5">
                                ...
                                <input
                                    ...
                                    placeholder="Substitute instructor name or ID"
                                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-800"
                                />
                                <p className="text-xs text-slate-400">Provide substitute tutor info for your classes</p>
                            </div>
                        </div>
