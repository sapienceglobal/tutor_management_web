...
const getGrade = (score) => {
  if (!score && score !== 0)
    return { letter: "N/A", color: C.text, bg: C.innerBg };
...
export default function TutorStudentPerformancePage() {
  ...
  const handleExport = async () => {
    try {
      setExporting(true);
      const res = await api.get("/tutor/dashboard/reports/students/export", {
        responseType: "blob",
      });
      ...
      link.setAttribute(
        "download",
        `Student_Performance_${new Date().toISOString().slice(0, 10)}.csv`,
      );
  ...
  const filteredStudents = useMemo(() => {
    return students
      .filter((s) => {
        ...
        const matchesCourse =
          courseFilter === "all" ? true : s.courses?.includes(courseFilter);
        return matchesSearch && matchesBatch && matchesCourse;
      })
      .sort((a, b) => {
        if (sortBy === "score")
          return (
            (b.indicators?.examAverage || 0) - (a.indicators?.examAverage || 0)
          );
        if (sortBy === "progress")
          return (b.indicators?.progress || 0) - (a.indicators?.progress || 0);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchInput, batchFilter, courseFilter, sortBy]);
  ...
  const avgScore = summary?.overallExamAverage ?? 0;
  const completedCourses = report?.courses?.completedCount ?? 0;
  const incompleteAssigns = summary?.totalPendingAssignments ?? 0;
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Tooltip
                ...
                cursor={{ stroke: `${C.btnPrimary}15`, strokeWidth: 2 }}
              />
              <Line
                type="monotone"
                dataKey="score"
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3 w-full lg:w-auto">
              ...
              <select
                ...
                style={{ ...baseInputStyle, width: 140, height: 38 }}
                onFocus={onFocusHandler}
                onBlur={onBlurHandler}
              >
            ...
            <div className="flex items-center gap-3 w-full lg:w-auto">
              ...
              <select
                ...
                style={{ ...baseInputStyle, width: 120, height: 38 }}
                onFocus={onFocusHandler}
                onBlur={onBlurHandler}
              >
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span
                          ...
                          style={{
                            ...
                            height: 24,
                            borderRadius: "10px",
                            backgroundColor: getGrade(progress).bg,
                            color: getGrade(progress).color,
                            fontFamily: T.fontFamily,
                            fontSize: T.size.xs,
                            fontWeight: T.weight.bold,
                          }}
                ...
                (page) => (
                  <button
                    ...
                    style={{
                      ...
                      height: 28,
                      backgroundColor:
                        currentPage === page ? C.btnPrimary : "transparent",
