...
export default function TutorAnnouncementsPage() {
    ...
    const TABS = [
        ...
        { id: 'notifications', label: `Notifications${unreadCount ? ` (${unreadCount})` : ''}`, icon: MdNotifications },
    ];
        ...
        <div className="w-full min-h-screen space-y-6" style={{ backgroundColor: C.pageBg, fontFamily: T.fontFamily, color: C.text }}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5" style={{ backgroundColor: C.cardBg, borderRadius: R['2xl'], border: `1px solid ${C.cardBorder}`, boxShadow: S.card }}>
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 flex items-center justify-center shrink-0" style={{ backgroundColor: C.iconBg, borderRadius: R.xl }}>
                        <MdCampaign size={24} color={C.iconColor} />
                    </div>
                    <div>
                ...
                <button
                    ...
                    disabled={refreshing}
                    className="flex items-center justify-center gap-2 h-10 px-5 cursor-pointer border-none transition-colors hover:bg-opacity-80 shadow-sm w-full sm:w-auto"
                ...
                >
                    {refreshing ? <MdHourglassEmpty size={16} className="animate-spin" /> : <MdRefresh size={16} />} Refresh
                </button>
            </div>

            {/* ── Main Tabs ─────────────────────────────────────────────── */}
            <div className="flex gap-2 p-1 w-full sm:w-max animate-in fade-in duration-500 delay-100" style={{ backgroundColor: C.innerBg, borderRadius: R.xl, border: `1px solid ${C.cardBorder}` }}>
                {TABS.map(tab => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)} className="flex-1 sm:flex-none flex items-center justify-center gap-2 h-11 px-6 cursor-pointer border-none transition-all relative"
                        ...
                        {tab.id === 'notifications' && unreadCount > 0 && (
                            <span className="absolute top-2 right-2 w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: C.danger }} />
                        )}
                    </button>
                ))}
            </div>
                        ...
                        {FILTER_TABS.map(({ key, label, count }) => {
                            ...
                            return (
                                <button key={key} onClick={() => setAnnouncementFilter(key)}
                                    ...
                                    style={{
                                        backgroundColor: isActive ? C.btnPrimary : C.innerBg,
                        ...
                        {filteredAnnouncements.length === 0 ? (
                            <div className="text-center py-20 flex flex-col items-center">
                                <div className="flex items-center justify-center mb-3" style={{ width: 56, height: 56, backgroundColor: C.innerBg, borderRadius: '10px' }}>
                                    <MdWarning size={28} color={C.textMuted} style={{ opacity: 0.5 }} />
                                </div>
                            ...
                            <div className="flex flex-col">
                                {filteredAnnouncements.map((item, index) => {
                                    ...
                                    return (
                                        <div key={`${item.sourceType}-${item.sourceId}-${index}`} className="p-5 transition-colors"
                                            style={{ backgroundColor: C.innerBg, borderBottom: index !== filteredAnnouncements.length - 1 ? `1px solid ${C.cardBorder}` : 'none' }}
                                            onMouseEnter={e => e.currentTarget.style.backgroundColor = C.btnViewAllBg}
                                            onMouseLeave={e => e.currentTarget.style.backgroundColor = C.innerBg}>
                                            ...
                                            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-3">
                                                <div>
                                                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                                                        <span className="flex items-center gap-1.5" style={{
                                                            fontSize: '10px', fontWeight: T.weight.black, padding: '4px 10px', borderRadius: R.md, textTransform: 'uppercase', letterSpacing: T.tracking.wider,
                                                            backgroundColor: sourceBg, color: sourceColor, border: `1px solid ${sourceBorder}`
                                                        }}>
                                                            {isCourse ? <MdMenuBook size={14} /> : <MdPeople size={14} />} {item.sourceType}
                                                        </span>
                                                ...
                                                <button onClick={() => router.push(isCourse ? `/tutor/courses/${item.sourceId}` : `/tutor/batches/${item.sourceId}`)}
                                                    className="flex items-center justify-center gap-2 h-9 px-4 cursor-pointer border-none transition-colors shrink-0 w-full sm:w-auto"
                                                    ...
                                                    Open Source <MdArrowForward size={14} />
                                                </button>
                                            </div>
                ...
                <div className="space-y-4 animate-in fade-in duration-500">
                    ...
                    <div className="flex justify-end">
                        <button onClick={markAllNotificationsRead} disabled={loadingNotifications || unreadCount === 0}
                            ...
                            {loadingNotifications ? <MdHourglassEmpty size={16} className="animate-spin" /> : <MdDoneAll size={16} />} Mark All Read
                        </button>
                    </div>
                    ...
                    <div className="overflow-hidden" style={{ backgroundColor: C.cardBg, borderRadius: R['2xl'], border: `1px solid ${C.cardBorder}`, boxShadow: S.card }}>
                        ...
                        {notifications.length === 0 ? (
                            <div className="text-center py-20 flex flex-col items-center">
                                <div className="flex items-center justify-center mb-3" style={{ width: 56, height: 56, backgroundColor: C.innerBg, borderRadius: '10px' }}>
                                    <MdNotificationsActive size={28} color={C.textMuted} style={{ opacity: 0.5 }} />
                                </div>
                            ...
                            <div className="flex flex-col">
                                {notifications.map((notif, index) => (
                                    <div key={notif._id} className="p-4 sm:p-5 transition-colors"
                                        style={{ 
                                            backgroundColor: notif.isRead ? C.innerBg : C.surfaceWhite, 
                                            ...
                                            <div className="flex flex-col sm:flex-row items-center gap-2 shrink-0">
                                                {!notif.isRead && (
                                                    <button onClick={() => markNotificationRead(notif._id)} title="Mark as read"
                                                        className="w-8 h-8 rounded-xl flex items-center justify-center cursor-pointer border-none transition-opacity hover:opacity-80 shadow-sm"
