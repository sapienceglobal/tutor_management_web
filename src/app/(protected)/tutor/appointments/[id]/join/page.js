...
export default function TutorJoinAppointmentPage({ params }) {
    const { id } = use(params);
    ...
    if (loading) return (
        <div className="h-screen w-full flex flex-col items-center justify-center gap-4" style={{ backgroundColor: '#111827' }}>
            ...
            <p style={{ fontFamily: T.fontFamily, fontSize: T.size.sm, fontWeight: T.weight.bold, color: '#9CA3AF' }}>Preparing Tutor Virtual Session...</p>
        </div>
    );
    ...
    return (
        <div className="h-screen w-full flex flex-col relative" style={{ backgroundColor: '#000' }}>
            ...
            <div className="absolute top-6 left-6 z-50 pointer-events-auto">
                <button
                    onClick={() => router.push('/tutor/appointments')}
            ...
            <div className="flex-1 w-full relative z-10">
                {meetingData.platform === 'zoom' ? (
                    <ZoomMeetingEmbed
                        config={meetingData}
                        onLeave={() => router.push('/tutor/appointments')}
                    ...
                    <JitsiMeeting
                        ...
                        userInfo={{
                            displayName: meetingData.userName || 'Tutor',
