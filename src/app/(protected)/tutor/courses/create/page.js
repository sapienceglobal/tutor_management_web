...
export default function CreateCoursePage() {
  ...
  const handleLessonVideoUpload = async (e) => {
    ...
    try {
      const res = await api.post("/upload/video-hls", fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      if (res.data?.success) {
        setLessonForm((prev) => ({
          ...prev,
          videoUrl: resolveUrl(res.data.estimatedPlaylistUrl),
        }));
      ...
      if (
        ...
        /(hls|feature|subscription)/i.test(msg)
      ) {
        try {
          const fd2 = new FormData();
          fd2.append("file", file);
          const fb = await api.post("/upload/file", fd2, {
            headers: { "Content-Type": "multipart/form-data" },
          });
          if (fb.data?.success || fb.data?.fileUrl) {
            setLessonForm((prev) => ({
              ...prev,
              videoUrl: fb.data.fileUrl || fb.data.url,
            }));
              ...
              {lessonForm.type === "video" && (
                <div className="space-y-3">
                  <input
                    ...
                    className="hidden"
                    id="tutor-lesson-video-upload"
                    disabled={isUploadingVideo}
                  />
                  <label
                    htmlFor="tutor-lesson-video-upload"
                    className={`flex items-center justify-center gap-2 w-full py-3 border-2 border-dashed cursor-pointer transition-all ${isUploadingVideo ? "opacity-60 cursor-wait" : ""}`}
                    style={{
                      borderColor: C.btnPrimary,
                      color: C.btnPrimary,
                      backgroundColor: C.innerBg,
                  ...
                  >
                    {isUploadingVideo ? (
                      ...
                      <>
                        <MdVideocam style={{ width: 14, height: 14 }} /> Upload
                        Video (Auto HLS)
                      </>
                    )}
                  </label>
